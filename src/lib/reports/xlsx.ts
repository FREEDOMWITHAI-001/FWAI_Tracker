...
function sheetDefinitions(wb: ExcelJS.Workbook, r: ReportResult) {
  ...
  const defs: [string, string][] = [
    ...
    ['Showed up', `Present in the attendance data with at least ${a.showed_up_min_minutes} minute(s) of watch time.`],
    ['Left early', `Showed up but watched less than ${a.left_early_minutes} minutes.`],
    ...
    ['₹0 orders', a.zero_order_value ? `100%-off / free orders are valued at a notional ₹${a.zero_order_value.toLocaleString()}.` : '100%-off / free orders count as conversions but contribute ₹0 to revenue.'],
